...
namespace YuGet.Core
{
    ...
    public static class PackageArchiveReaderExtensions
    {
        ...
        {
            if (string.IsNullOrEmpty(authors)) return new List<string>();

            return authors.Split(new[] { ',', ';', '\t', '\n', '\r' }, StringSplitOptions.RemoveEmptyEntries).ToList();
        }

        private static ICollection<PackageTag> ParseTags(string tags)
        {
            if (string.IsNullOrEmpty(tags)) return new List<PackageTag>(0);

            return tags.Split(new[] { ',', ';', ' ', '\t', '\n', '\r' }, StringSplitOptions.RemoveEmptyEntries)
                .Select(a => new PackageTag  { Tag = new Tag { Name = a } }).ToList();
        }
