...
namespace YuGet.Core.Internal
{
    public class RawCatalogClient : ICatalogClient
    {
        ...
        {
            return await _httpClient.GetFromJsonAsync<CatalogIndexRef>(_catalogUrl, cancellationToken);
        }

        public async Task<CatalogPage> GetPageAsync(string pageUrl, CancellationToken cancellationToken = default)
        {
            return await _httpClient.GetFromJsonAsync<CatalogPageRef>(pageUrl, cancellationToken);
        }
        ...
        {
            return await GetAndValidateLeafAsync<PackageDeleteCatalogLeafRef>(
                "PackageDelete",
                leafUrl,
                cancellationToken);
        }
        ...
        {
            return await GetAndValidateLeafAsync<PackageDetailsCatalogLeafRef>(
                "PackageDetails",
                leafUrl,
                cancellationToken);
        }
